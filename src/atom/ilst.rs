...
mod tests {
    ...
    fn test_malformed_metadata_parsing() {
        ...
        let publisher_text = b"Example Publisher";
            ...
            Ok(ilst) => {
                ...
                if let Some(publisher_item) = ilst.get_first_item(&pub_key) {
                    match &publisher_item.value {
                        MetadataValue::Text(text) => {
                            assert_eq!(text, "Example Publisher", "Text should not be truncated");
                            assert_eq!(
    ...
    fn test_edge_case_data_size_calculations() {
        ...
        let text_data = b"Example Publisher";
    ...
    fn test_multiple_data_atoms_in_item() {
        ...
        let mut data = Vec::new();
        let item_type = b"\xa9pub";
        let text_data1 = b"Example";
        let text_data1 = b" Publisher";
        ...
        match result {
            Ok(ilst) => {
                ...
                if let Some(item) = ilst.get_first_item(&pub_key) {
                    if let MetadataValue::Text(text) = &item.value {
                        // Should be "Example", not corrupted
            ...
            Ok(ilst) => {
                ...
                if let Some(item) = ilst.get_first_item(&pub_key) {
                    if let MetadataValue::Text(text) = &item.value {
                        ...
                        assert!(
                            text.starts_with("Example Publisher")
                                || text.trim_end_matches('\0') == "Example Publisher"
                        );
