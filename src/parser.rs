        ...
        while length_limit.map_or(true, |limit| self.current_offset - start_offset < limit) {
            ...
            let (size, header_size) = if size_32 == 1 {
                ...
                if let Some(limit) = length_limit {
                    ...
                    let remaining = if limit > bytes_read {
                        limit - bytes_read
                    } else {
                        0
                    };
            ...
            let atom_data = if is_container_atom(&atom_type) {
                // For container atoms, store raw data for child parsing
                None
            } else {
                ...
                match &atom_type {
                    FTYP => Some(
                        FileTypeAtom::try_from(complete_atom_data.as_slice())
                    ...
                    MVHD => Some(
                        MovieHeaderAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    ELST => Some(
                        EditListAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    MDHD => Some(
                        MediaHeaderAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    HDLR => Some(
                        HandlerReferenceAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    GMHD => Some(
                        GenericMediaHeaderAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    SMHD => Some(
                        SoundMediaHeaderAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    TKHD => Some(
                        TrackHeaderAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    STSD => Some(
                        SampleDescriptionTableAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    TREF => Some(
                        TrackReferenceAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    DREF => Some(
                        DataReferenceAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    STSZ => Some(
                        SampleSizeAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    STCO | CO64 => Some(
                        ChunkOffsetAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    STTS => Some(
                        TimeToSampleAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
                    ...
                    STSC => Some(
                        SampleToChunkAtom::try_from(complete_atom_data.as_slice())
                            .map_err(|e| ParseError {
                                kind: ParseErrorKind::AtomParsing,
                                location: Some((atom_offset as usize, complete_atom_data.len())),
                                source: Some(e.context(atom_type_fourcc).into()),
...
fn is_container_atom(atom_type: &[u8; 4]) -> bool {
    // Common container types in MP4 might include 'moov', 'trak', 'mdia', and 'minf'.
    matches!(
        atom_type,
        b"moov"
            | b"mfra"
